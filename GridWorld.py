class GridWorld:
    ...
    def policy_evaluation(self, iteration:int, gamma:float, step_cost:float, total_step:int) -> int:
        for _ in range(iteration):
            ...
            for y in range(self.y):
                for x in range(self.x):
                    ...
                    if (x, y) in self.states_reward:
                        new_world[y][x] = self.states_reward.get((x, y))
                    elif step_cost:
                        new_world[y][x] = step_cost
                    else:
                        new_world[y][x] = self.world[y][x]
                    new_world[y][x] += gamma * (sum(directions) / len(directions))
    ...
    def value_iteration(self, gamma:float, step_cost:float, print_status:bool) -> None:
        ...
        while True:
            iteration += 1
            print(f"Iteration: {iteration}")
            total_step = self.policy_evaluation(1, gamma, step_cost, total_step)
            stable, total_step = self.policy_improvement(total_step)
            if stable:
                print("Policy Stabled")
                print(f"Total step: {total_step}")
