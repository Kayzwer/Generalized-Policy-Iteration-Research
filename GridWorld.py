class GridWorld:
    ...
    def set_terminal_reward_state(self, x:int, y:int, r:float) -> None:
        self.world[y][x] = r
        self.policy[y][x] = []
        self.terminal_states.append((x, y))
    ...
    def policy_evaluation_for_policy_iteration(self, gamma:float, step_cost:float, theta:float, total_step:int) -> int:
        while True:
            total_step += 1
            delta = 0
            new_world = [[x for x in y] for y in self.world]
    ...
    def policy_iteration(self, gamma:float, step_cost:float, theta:float, print_status:bool) -> None:
        ...
        while True:
            ...
            total_step = self.policy_evaluation_for_policy_iteration(gamma, step_cost, theta, total_step)
    ...
    def value_iteration(self, gamma:float, step_cost:float, theta:float, print_status:bool) -> None:
        total_step = 0
        iteration = 0
        while True:
            iteration += 1
            print(f"Iteration: {iteration}")
            total_step = self.policy_evaluation(1, gamma, step_cost, theta, total_step)
            stable, total_step = self.policy_improvement(total_step)
            if print_status:
                print(self)
                print(self.policy_to_str())
    ...
    def SHIN_value_iteration(self, n:int, gamma:float, step_cost:float, theta:float, print_status:bool) -> None:
        total_step = 0
        iteration = 0
        while True:
            iteration += 1
            print(f"Iteration: {iteration}")
            total_step = self.policy_evaluation(n, gamma, step_cost, theta, total_step)
            stable, total_step = self.policy_improvement(total_step)
            if print_status:
                print(self)
                print(self.policy_to_str())
